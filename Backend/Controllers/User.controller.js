  ...
  try {
    ...
    if (req.body.flag) {
      let resp = await User.updateOne(
        { _id: req.body.id },
        { $set: { Points: user.Points + req.body.points } }
      );
      const updatedPoints = await User.findOne({ _id: req.body.id }).select(
        "Points"
      );
...
async function getParentStudent(req, res) {
  try {
    let student = await Parent_Student.findOne(
...
async function getAttendance(req, res) {
  try {
    ...
    if (attendance.length === 0) {
      let students = await Class_Student.find(
        { Class_id: req.body.class_id },
        { Student_id: 1, _id: 0 }
      );

      let studentIds = [];
      students.map((i) => studentIds.push(i.Student_id));
  ...
  try {
    if (req.body.username) {
      let user = await User.find(
        { Name: new RegExp(req.body.username) },
        { pfp: 1, Name: 1, _id: 1 }
      );
      res.status(200).send(user);
    } else {
...
async function getGroups(req, res) {
  ...
  try {
    let groupDetails = [];
    let groups = await User.findOne(
      { _id: req.body.id },
      { Groups: 1, _id: 0 }
    );

    await Promise.all(
      groups.Groups.map(async (g, i) => {
...
async function getGroupMessages(req, res, next) {
  try {
    let currentUser = req.body.user;
    let groupMessages = await Message.find({
      $and: [{ Type: true }, { ReceiverId: { $in: [currentUser] } }],
    });
