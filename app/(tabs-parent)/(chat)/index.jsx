...
const index = () => {
  ...
  async function checkContacts()
  {
    try
    {
      ...
      let teachers = await axios.post(`${API.BASE_URL}/api/getTeachersfromClasses`,ClassIds)

      teachers = teachers.data

      let TeacehrIds =[]
      teachers.map((i)=>(
        TeacehrIds.push(i.Teacher_id)
      ))

      const uniqueTeacherIds = [...new Set(TeacehrIds)];
      ...
      uniqueTeacherIds.forEach(Class => {
        let flag = true
        
        for(let element = 0 ; element < contacts.length ; element++)
        {
          if(contacts[element].contactId._id == Class )
          {
            flag = false
            break
          }
